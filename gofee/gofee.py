...
from gofee.surrogate import GPR    ###SAM, The class has been modified
...
from gofee.bfgs_constrained import relax   ###SAM, Changed to bfgs since it is more stable than bfgslinesearch
...
class GOFEE():
    ...
    def __init__(self, structures=None,
                 ...
                 ###SAM, Additional input to check similarity. Pls be carefull when choosing the similarity_thr.
                 chk_similarity=True,
                 similarity_thr=1e-3):
        ...
        ###SAM, Additional input
        self.chk_similarity = chk_similarity
        ...
        #self.population_method = 'similarity'
            ...
            for _ in range(self.Ncandidates):
                try:
                    ...
                    if idx_relax is not None:
                        ...
                        similar, anew, adp = self.check_similarity_will_all_runs(anew0)
                        if similar is False:
                    ...
                    #kappa /=2
                    relaxed_candidates.remove(anew0)
                    if self.master:
                        print(f'Exception :\n{err}', file=sys.stderr)
    ...
    def generate_new_candidates(self):
        ...
        if self.Ncandidates > len(self.unrelaxed_candidates):
            Njobs = self.Ncandidates - len(self.unrelaxed_candidates)
            task_split = split(Njobs, self.comm.size)
            candidates = parallel_function_eval(self.comm, func1)
            for i in candidates:
                if i is not None:
                    self.unrelaxed_candidates.append(i)
            self.generate_new_candidates()
    ...
    def get_dir(self):
       #### 2023/08/18: Get the directory containing gofee calculation
       os.chdir(self.parent_path)
       list_directory = []
       list_directory.append(os.path.abspath('.'))
       
       os.chdir('..')
       all_dirr = os.listdir('.')
       random.shuffle(all_dirr)
       for i in all_dirr:
               if i.find('run') != -1:
                   list_directory.append(os.path.abspath(i))
       if len(list_directory) > self.comm.size: 
            remove_dir = len(list_directory) - self.comm.size
            list_directory = list_directory[:-remove_dir]
       list_directory = set(list_directory)
       list_directory = list(list_directory)

       if len(list_directory) < self.comm.size:
           for i in range(self.comm.size - len(list_directory)):
               list_directory.append(list_directory[0])
       
       os.chdir(self.parent_path)
       
       return list_directory

    def check_similarity_will_all_runs(self, a):
        #### 2023/08/18: Check similary on paralel runs. The job are devided between available cores. 
        if self.chk_similarity == False:
        ...
        list_directory = self.get_dir()
        #self.log_msg += (f"List dir {list_directory}\n")
        #self.log()

        Njobs = len(list_directory)
        task_split = split(Njobs, self.comm.size)
        #self.log_msg += (f"task split = {task_split} \n")
        #self.log_msg += (f"task split self.comm.rank = {task_split[self.comm.rank]} \n")
        #self.log()
        def func3():
            return [self.check_similarity(list_directory[i], f1=f1) for i in task_split[self.comm.rank]]
        
        #[anew_copy0, adp_copy0, dir] = parallel_function_eval(self.comm, func3)
        paralel_result = parallel_function_eval(self.comm, func3)
        #self.log_msg += (f"Paralel result {paralel_result}\n")
        #self.log()

        anew_copy0, adp_copy0, dir = [], [], []
        anew_copy, adp_copy = None, None

        for i in paralel_result:
            if i[0] is not None:
                anew_copy0.append(i[0])
                adp_copy0.append(i[1])
                dir.append(i[2])

        try:
            index_lowest0 = np.argmin([a.get_potential_energy() for a in anew_copy0])
            anew_copy = anew_copy0[index_lowest0]
        except:
            pass
        
        try:
            index_lowest1 = np.argmin([a.get_potential_energy() for a in adp_copy0])
            adp_copy = adp_copy0[index_lowest1]
        except:
            pass
        
        if anew_copy is not None:
            self.log_msg += (f"Similar structure was found and copied from {dir[index_lowest0]}.\n") # and {dir[index_lowest1]} \n")
            #self.log()
            self.write(anew_copy)
            self.write(adp_copy)
            return True, anew_copy, adp_copy
        else:
            return False, None, None


    def check_similarity(self, dir, f1=None):
        #### 2022/11/18: Read all trajectory and calculate the kernel value
        os.chdir(dir)
        #self.log_msg += (f"Similarity in {dir} \n")
        #self.log()
        ...
        try:
            ...
            if number_of_structures >= 50:
                for j in range(number_of_structures-49, number_of_structures):                        
                    f2 = self.gpr.descriptor.get_feature(structures[j])
                    similarity = 0.5 * abs(1 - np.dot(f1,f2) / (np.sqrt(np.dot(f1,f1)) * np.sqrt(np.dot(f2,f2))) )
                    #self.log_msg += (f"Similarity = {similarity} \n")
                    #self.log()
                    if similarity < self.similarity_thr:
                        anew_copy0 = structures[j]
                        if j % 2 == 0:
                            adp_copy0 = structures[j+1]
                        else:
                            adp_copy0 = structures[j-1]
                        
                        os.chdir(self.parent_path)
                        return [anew_copy0, adp_copy0, dir]
        except:
            pass 
        os.chdir(self.parent_path)       
